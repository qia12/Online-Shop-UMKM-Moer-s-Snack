...
class Shop extends MY_Controller
{
    ...
    public function index($page = null)
    {
        ...
        $data['content']    = $this->shop->select(
            [
                ...
                'product.price', 'product.is_available', 'product.weight',
    ...
    public function search($page = null)
    {
        if (isset($_POST['keyword'])) {
            ...
            redirect(base_url('pages/shop/index'));
        }

        $keyword    = $this->session->userdata('keyword');
        $data['title']        = 'Pencarian: Produk';
        $searchResults = $this->shop->select([
            'product.id', 'product.title AS product_title',
            'product.description', 'product.image',
            'product.price', 'product.is_available',
            'category.title AS category_title', 'category.slug AS category_slug'
    ...
    public function detail($id)
    {
        ...
        $data['content']    = $this->shop->where('id', $id)->first();
        if (!$data['content']) {
            $this->session->set_flashdata('warning', 'Data tidak ditemukan.');
            redirect(base_url('/shop'));
        }

        $this->shop->table    = 'product';
        $data['product']    = $this->shop->select([
